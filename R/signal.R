...
#'
#' @export
iSensor_signal_old <- function(iSensor_obj, panels = NULL, transform, normed = FALSE, normBy = 'cols',
...
iSensor_signal <- function(iSensor_obj, transform = "mean",
  ...
  signalList <- lapply(panelNames, function(pName) {
    compute_signal(iSensor_obj$exprData, panelsList[[pName]], transform, normed, normBy)
